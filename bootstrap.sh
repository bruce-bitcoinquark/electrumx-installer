...
if which git > /dev/null 2>&1; then
    git clone https://github.com/bruce-bitcoinquark/electrumx-installer ~/.electrumx-installer
else
    which wget > /dev/null 2>&1 && which unzip > /dev/null 2>&1 || { echo "Please install git or wget and unzip" && exit 1 ; }
    wget https://github.com/bruce-bitcoinquark/electrumx-installer/archive/master.zip -O /tmp/electrumx-master.zip
